    ...
    init {
        ...
        mTextPaint.run {
            ...
            typeface = when(mTypefaceIndex) { // for the constant values please check the WheelPicker_typeface in the attrs.xml
                ...
                2 -> Typeface.SERIF
                else -> ResourcesCompat.getFont(context, R.font.nunito_semi_bold)
            }
        }
        ...
        while (i < mSelectorItemIndices.size) {
            ...
            if (mSelectorItemValidStatus[i]) {
                if (offsetToMiddle < mItemHeight / 2) {
                    mTextPaint.color = context.getColor(R.color.txt_dark)
                } else {
                    mTextPaint.color = context.getColor(R.color.txt_description)
                }
            } else {
                mTextPaint.color = ContextCompat.getColor(context, R.color.txt_description)
            }

            canvas.save()
            canvas.scale(scale, scale, x, y)
            canvas.drawText(getValue(mSelectorItemIndices[i]), x, 40 + y - 20, mTextPaint)
            canvas.restore()
